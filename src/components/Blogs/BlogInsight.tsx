          ...
          <h2 className="font-semibold text-2xl md:text-3xl leading-8 md:leading-10 ">
            ...
            <div className="mt-10 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 ">
              {articles.map((article, index) => (
                <ArticleCard key={index} img={article.img} date={t(`articles.${index}.date`)} title={t(`articles.${index}.title`)} description={t(`articles.${index}.description`)} link={article.link} />
              ))}
